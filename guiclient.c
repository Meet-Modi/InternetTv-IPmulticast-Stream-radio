...
void *radio_channel(void *input){
  ...
  if (setsockopt(multi_sockfd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt, sizeof(opt))) 
    { 
        perror("setsockopt"); 
        exit(EXIT_FAILURE); 
    } 

  if ((setsockopt(multi_sockfd, SOL_SOCKET, SO_BINDTODEVICE, (void *)&ifr, sizeof(ifr))) < 0){
...
/*-----------------------------------------------------------------*/
  ...
  while(flag){
    if(pause_flag == 0){
      ...
      if((recieve_size = recvfrom(multi_sockfd, buffer,BUF_SIZE,0,(struct sockaddr*)&mcast_servaddr,&mcast_servaddr_len)) < 0){
...
*/
   // flag = 0;
    ...
    station->multicast_address = slist.station_list[curr_radio_channel].multicast_address;
...
void button_fetch_clicked( gpointer data ){
  ...
  for(int i = 0; i < slist.station_count; i++){
    ...
    snprintf(temp, sizeof(temp), "%d", slist.station_list[i].multicast_address);
    printf("temp : %s\n", temp);
    radiolist[i][2] = temp;
...
int main(int argc, gchar *argv[]) {       
  ...
  gtk_signal_connect_object(GTK_OBJECT(button_fetch), "clicked",GTK_SIGNAL_FUNC(button_fetch_clicked),(gpointer) clist);
