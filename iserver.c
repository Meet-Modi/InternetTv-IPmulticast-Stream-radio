...
void *threadfunctionUDP(void *input) {
	...
	double execTime;
 	...
 	FILE *duration_file;
 	duration_file = fopen(duration_file_name, "r");
 ...
 */	
	...
	char sys_call[] = "ffmpeg -i station1.mp4  2>&1 | grep Duration | cut -d ' ' -f 4 | sed s/,// > duration_1.txt";
 	system(sys_call);
	strcpy(sys_call,"ffmpeg -i station2.mp4  2>&1 | grep Duration | cut -d ' ' -f 4 | sed s/,// > duration_2.txt");
 	system(sys_call);
 	strcpy(sys_call,"ffmpeg -i station3.mp4  2>&1 | grep Duration | cut -d ' ' -f 4 | sed s/,// > duration_3.txt");
 	system(sys_call);
